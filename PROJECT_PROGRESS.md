...
- **Header count reuses the cart badge component, not a copy of it.** `snippets/cart-bubble.liquid` gained three optional params — `count` (defaults to `cart.item_count`), `interactive` (defaults true) and `testid` — so a non-cart counter can render the same markup. `interactive: false` omits `ref="cartBubble"/"cartBubbleText"/"cartBubbleCount"` and `id="cart-bubble-text"`, which would otherwise be duplicated in the DOM and get picked up by `cart-icon.js`'s ref lookup. Default output for the cart is unchanged (`cart == empty` became the equivalent `item_count == 0`); verified live that the cart bubble still carries all three refs, the single id, hides at 0 and counts correctly.
- `<wishlist-count>` now wraps that rendered bubble and drives it the way `cart-icon.js` drives the cart's: `visually-hidden` at zero, blank text above 99 (so the CSS shrinks it to a dot), `header-actions__wishlist--has-items` toggling the same donut mask over the icon, and the same `cart-bubble--animating` pulse — awaited through the theme's own `onAnimationEnd` utility — only when the count grows. No second count state: it still reads `WishlistStore`, so cross-tab sync is unchanged.
- Header CSS mirrors `.header-actions__cart-icon`: badge over the icon on mobile, inline after the label on desktop in text mode, same `--cart-bubble-size/top/right` custom properties.
- **Fly-to-cart needed no new code at all.** The animation is driven by `snippets/add-to-cart-button.liquid` (`data-add-to-cart-animation`, `data-product-variant-media`) and `#animateFlyToCart()` in `assets/product-form.js`, which targets `.header-actions__cart-icon`. Wishlist cards render the theme's own `quick-add`, so they already carried all of it. Confirmed live on the wishlist page: a `<fly-to-cart class="fly-to-cart--quick">` is created with the product image as its background, the travel distance is computed to the header cart icon (858.59px at 1280), the element removes itself, and the correct product lands in the cart.
- A `{% render 'fly-to-cart-styles' %}` was added to `sections/wishlist.liquid` and then **removed again after testing**: the animation CSS is already present on that page, so the line was dead weight. Confirmed by deleting it and re-checking that a probe `<fly-to-cart>` still computes `position: fixed` and `animation-name: travel-x, travel-y, travel-scale`. The other card style renders in that section are *not* redundant — with them removed, `product-card` falls back to `display: inline`.
- Verified live: badge 0 → 1 → 2 → 3 → 2 → 0 with hide/show at zero; count survives reload; cross-tab still syncs; computed styles of the wishlist and cart bubbles differ in **zero** properties (width, height, border-radius, background, font, colour, position all identical, both 20×20); mobile shows the badge over the heart with the donut mask, desktop shows it inline after the "Wishlist" label.
- Malformed `localStorage` — non-JSON, object, number, `null`, empty string, array of junk, nested arrays, and 500 entries — produced no exceptions and no console errors; every bad value reads as an empty wishlist, and the 500-entry case renders as the dot (>99) and is capped at 100 on the next write.
- `shopify theme check`: still the project baseline of 6 pre-existing `ValidScopedCSSClass` warnings in the teammate's `sections/collection-list.liquid`; 0 wishlist offenses.
- Still outstanding, unchanged from Phase 7: `/pages/wishlist` renders the default page template until the page's Theme template is set to `wishlist` in Admin (only selectable once this theme is published), and the sold-out card has never rendered because all 19 products are in stock. The sold-out branch in `sections/wishlist-card.liquid` was diffed against `blocks/_product-card-gallery.liquid` and is **byte-identical**, including the `product.available` quick-add guard.

### Phase 7 — Wishlist (localStorage, no app, no backend)
- **Storage.** One module, `WishlistStore` in `assets/wishlist.js`, is the only thing that touches persistence — key `paridhi:wishlist`, value a JSON array of **product handles only** (never product objects), capped at 100 entries. Every read is defensive: non-array or malformed JSON returns `[]`, and `localStorage` failures (Safari private mode, quota) are swallowed so the session keeps working un-persisted. Because only handles are stored, price/availability/images always come from live Shopify data and can never go stale. Swapping this module for customer-account metafields later requires no UI changes.
- **Sync.** `WishlistStore.set()` dispatches a `wishlist:change` `CustomEvent` on `document`; `subscribe()` also listens to the window `storage` event, so a change in one tab updates every other open tab. All three components subscribe, so the header count, every card heart and the wishlist page stay consistent without polling.
- **Components** (plain custom elements, no framework): `<wishlist-button>` toggle, `<wishlist-count>` header badge (hidden at 0, caps at `99+`), `<wishlist-page>` grid resolver.
- **No duplicated card markup.** The wishlist page holds handles, not products. `assets/wishlist.js` fetches `/products/{handle}?section_id=wishlist-card` per saved handle through the **Section Rendering API**, and `sections/wishlist-card.liquid` renders the theme's own `_product-card` block. Cards therefore arrive with the real price, compare-at, sale/sold-out badges and quick add. A deleted or unpublished product returns an empty section, and the page silently prunes that handle from storage.
- **Heart placement on cards.** Injected into the gallery block's `children` capture in `blocks/_product-card-gallery.liquid`, positioned in the corner the badge is *not* using (`settings.badge_position == 'top-right'` puts the heart top-left). Necessary because every client product currently carries a sale badge, so a fixed top-right heart would always collide. Badge placement itself is untouched.
- **Both icon states are rendered in Liquid** (`icon-heart.svg` + `icon-heart-filled.svg`) and CSS swaps which is visible — toggling never fetches and never shifts layout, and the markup stays cacheable. State is applied by JS on `connectedCallback`.
- **Click isolation.** Product cards wrap the whole tile in an `<a>`, so the heart's handler calls `preventDefault()` + `stopPropagation()`; tapping the heart never navigates.
- **Accessibility.** Real `<button>` with `aria-pressed` reflecting saved state and an `aria-label` that swaps between "Add X to wishlist" / "Remove X from wishlist"; the wishlist page has a `role="status" aria-live="polite"` region announcing the item count; the clear action is confirmed before it destroys anything; touch targets use `--minimum-touch-target`.
- Files added: `assets/wishlist.js`, `assets/icon-heart.svg`, `assets/icon-heart-filled.svg`, `snippets/wishlist-button.liquid`, `snippets/wishlist-button-styles.liquid`, `blocks/wishlist-button.liquid`, `sections/wishlist.liquid`, `sections/wishlist-card.liquid`, `templates/page.wishlist.json`.
- Files changed: `blocks/_product-card-gallery.liquid` (heart in the gallery children), `snippets/header-actions.liquid` (wishlist link + count, gated on `settings.show_wishlist`), `snippets/scripts.liquid` (module script), `config/settings_schema.json` (new "Wishlist" group: `show_wishlist`, `wishlist_page_handle`), `templates/product.json` (wishlist button block after Add to cart).
- **No translation keys were added.** An earlier pass added nine `t:` keys to `locales/en.default.json`, which made `MatchingTranslations` fail with 270 errors across the 30 other locale files. Rather than ship guessed translations into 30 languages for an English-only store, the keys were reverted and the wishlist strings are literals in Liquid — consistent with the wishlist section's schema, which is already plain English. Merchant-facing copy (headings, empty state, clear-confirmation message) is editable through section settings. `locales/en.default.json` is unmodified.
- **The card section composes its own children.** First attempt used a static `_product-card` block with the child blocks declared in the section's `presets`. It returned a 1,787-byte card with no children: the Section Rendering API renders a section that is in no template with **no block tree at all**, and presets are not applied, so `content_for 'blocks'` was empty. `sections/wishlist-card.liquid` now renders `card-gallery`, the title link and `price` directly, and holds no blocks or presets. Same underlying snippets as the collection grid, so the cards still match; dropping the preset also keeps this API-only section out of the "Add section" picker.
- **Two rendering bugs found and fixed in the browser:**
  - The empty state showed underneath a populated grid. `.wishlist__empty { display: flex }` outranks the user-agent `[hidden] { display: none }` rule. Added an explicit `.wishlist [hidden] { display: none }`, which also covers the grid, actions and loading row.
  - The page had no horizontal padding. `.wishlist { display: flex }` was overriding Horizon's `.section` grid, and that grid is what places children in the page-width column (`.section--page-width > * { grid-column: 2 }`). The flex column moved to a new inner `.wishlist__inner` wrapper; content now insets to 40px at 1280.
- **Dead-handle pruning was broken and is fixed.** A missing product answers **HTTP 200 with the section wrapper around nothing** (71 bytes), not an empty body, so the old `html.length` check treated it as a real card and injected a blank tile. `#fetchCard` now parses the response and treats it as missing unless it contains a `<product-card>`. It also strips the section wrapper's `id`, which was otherwise duplicated once per card.
- Verification — `shopify theme check`: **0 wishlist offenses**, total back to the project baseline of 6 pre-existing `ValidScopedCSSClass` warnings in the teammate's `sections/collection-list.liquid`. (Fixed along the way: `{% doc %}` is not allowed in sections; `blocks_order` is not a section schema key; section default blocks belong under `presets`; static blocks must not appear in a preset `block_order`; `blocks/price.liquid` has no `max_width`/`wrap`/`background`/`background_color`/`corner_radius`.) `sections/wishlist-card.liquid` carries a scoped `theme-check-disable ValidScopedCSSClass`, because it deliberately uses classes owned by `sections/wishlist.liquid` — its own `{% stylesheet %}` can never reach the wishlist page, since its HTML gets there by fetch.
- Verified live via `shopify theme dev` against dev theme `#200769634385` (storefront password supplied on the command line):
  - Collection page: 19 hearts, all upgraded custom elements. Clicking one adds without navigating (`/collections/all` unchanged), sets `aria-pressed="true"`, swaps the label to "Remove … from wishlist", and moves the header count 0 → 1. Hearts sit top-left because every product carries a top-right Sale badge.
  - PDP: inline "♡ Save" under Buy it now toggles to "Saved", `aria-pressed="true"`, count 3 → 4, no navigation.
  - Wishlist page: renders the theme's real cards — image, title, price with compare-at strikethrough, sale badge, quick add. Live region reads "N items in your wishlist".
  - Add to cart from a wishlist card: cart 2 → 3, header bubble updated.
  - Remove from the wishlist page: card disappears in place without refetching the others; count and live region follow.
  - Clear wishlist: confirm dialog reads "Remove all items from your wishlist?"; Cancel leaves 2 items, OK empties storage, hides grid and actions, hides the header count and shows the empty state.
  - Dead handle: an invented handle is dropped from storage on the next render, leaving the real 4 and no blank tile.
  - Cross-tab: adding from a second tab moved the first tab's header count to 5 and pulled the new card into the open wishlist page.
  - Persistence holds across navigation and reloads.
  - Breakpoints — 2 columns at 320/375/390, 3 at 768/1024, 4 at 1280/1440, **0px horizontal overflow at every width**; hearts are 44×44 at 320px. Mobile header (375px) shows the heart icon plus count and hides the text label; desktop shows "Wishlist".
  - Console: no wishlist errors. The page's only console errors are pre-existing and environmental — analytics CORS against `/api/collect` from `127.0.0.1`, and a missing `customer-account-main-menu`.
- **Not verified: the sold-out card.** All 19 products are in stock, so `product.available == false` never rendered. The badge and quick-add branches are the same Liquid the collection card uses (the Sale branch of that block did render), but it has not been seen live.
- **`/pages/wishlist` still needs its template assigned.** The page exists with handle `wishlist`, but Admin's Theme template dropdown lists templates from the *published* theme, and `page.wishlist.json` is only on the dev theme — so it is currently set to "Default page" and the bare URL renders an empty page. All live testing used `/pages/wishlist?view=wishlist`, which forces the alternate template. Once this branch is merged and the theme published, set Template to `wishlist` on that page and the plain URL works.

### Phase 6 — Product page architecture (root-caused + rebuilt)
- **Why the info sat under the gallery on desktop.** Horizon's product grid did have two columns (793px / 397px at 1280), but `product-component` had a *third* child: an empty `<strong>` occupying column 2 of row 1, which pushed `.product-details` to row 2, column 1. It came from the previous `snippets/product-short-desc.liquid`, which split the description HTML on the text `Jewellery care:` and echoed the first half — ending in an unclosed `<p><strong>`. The HTML parser reconstructs open formatting elements after the enclosing div closes, so a stray `<strong>` became a blockified grid item. The same file also caused the duplicated description, and `snippets/product-care.liquid` was a 0-byte file, so the Care accordion rendered empty while the care text showed inline in the full-description block.
- **Restored** `blocks/_product-details.liquid` to stock Horizon (removed the hardcoded `<details>` markup and hex-coloured CSS) and deleted `product-short-desc`, `product-care`, `product-materials`, `product-occasion`, `product-size-fit` snippets (the last three also read the wrong metafield keys — `colour_finish` / `occasion_style` instead of the store's `color_finish` / `ocassion_style`).
- **New reusable block `blocks/product-info.liquid`** ("Product info", addable in the Theme Editor inside Product details). Settings: heading; content source = description / size & fit / materials & finish / occasion & style / care instructions / shipping & returns (store policies) / any metafield (`namespace.key`) / custom text; display = accordion or plain text; open by default; plus/caret icon. Renders **nothing** when the resolved content is empty (no empty accordions), with a muted placeholder in the editor so the block stays selectable. Uses Horizon's `accordion-custom-component` and `accordion-styles`; headings use the h6 preset variables; no hardcoded colours or sizes.
- **New snippet `snippets/product-description-parts.liquid`**: splits the description *per `</p>` paragraph* into body vs any paragraph mentioning "jewellery care" — never cuts a tag open. Care accordion reads `custom.care_instructions`, falling back to the extracted paragraph (label stripped). Short description = body only. Result: care text appears exactly once, inside CARE INSTRUCTIONS.
- **Section setting `media_width`** (50–70%, default 58) added to `sections/product-information.liquid`; `snippets/product-information-content.liquid` emits `--product-grid-columns` and applies it from 990px via `.product-information__grid--ratio`. Only the product-information section emits it; the featured-product section keeps the stock grid.
- **Template `templates/product.json`**: gallery → square (`aspect_ratio: 1`), 2-column grid, viewport constraint off; details gap 24, top padding 0 (title aligns with gallery top); right column = title + price + review (self-hiding until a reviews app fills `reviews.rating`) → divider → variant picker → quantity + Add to cart + accelerated checkout → short description (inline product-info) → group (gap 0) of accordions: Size & fit, Materials & finish, Occasion & style, Care instructions, Shipping & returns (policies; hidden until the store publishes them). Removed the full-description text block.
- **Back link**: `layout/theme.liquid` now renders `<a href="{{ routes.root_url }}" aria-label="Back to <shop>">` — no `history.back()` / `document.referrer`; the JS click handler was removed from `custom.js`. Product A → Product B → Back always lands on `/`.
- Verified live (dev theme via `shopify theme dev`): 1280px — grid has exactly 2 children, 690px/500px (58%), details in column 2, sticky; 4 square 332px gallery tiles; care text 0 occurrences while accordion closed, 1 inside it when open, `Jewellery care:` label absent; description once; `long-raani-haar` (no care data) shows no Care accordion. 375px — gallery becomes a 375×375 carousel with dots, info below, Add to cart 52px, sticky ATC bar, accordion rows 46px, back link 44px, overflow 0. 320px — overflow 0. `shopify theme check`: my files clean; 6 pre-existing `ValidScopedCSSClass` warnings remain in the teammate's `sections/collection-list.liquid`.
- Notes: dev theme `199608631377` no longer exists on the store (deleted by someone) — the teammate's `shopify theme dev` created a new one. The client's product text is ALL CAPS at source; rendered as-is. `choker-set-with-earrings` has a "THIS SET CONSISTS OF…" sentence stored inside its care metafield — a data quirk to move into the description in Admin. Shipping & returns accordion appears automatically once the shipping/refund policies are published.
